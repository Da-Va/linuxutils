...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey,           }, "#59", function () awful.screen.focus_relative( 1) end,
              {description = "focus the next screen", group = "screen"}),
    awful.key({ modkey,           }, "#60", function () awful.screen.focus_relative(-1) end,
    ...
    awful.key({}, "XF86MonBrightnessDown", function () awful.util.spawn({"light", "-U", "2"}, false) end,
...
clientkeys = gears.table.join(
    ...
    awful.key({ modkey,   "Shift" }, "#59",      function (c) c:move_to_screen(c.screen.index-1)               end,
              {description = "move to screen", group = "client"}),
    awful.key({ modkey,   "Shift" }, "#60",      function (c) c:move_to_screen(c.screen.index+1) end,
              {description = "move to screen", group = "client"}),
...
client.connect_signal("manage", function (c)
    ...
    c.size_hints_honor = false  -- Fixes size of terminals
...
-- Title bar for floating only
-- client.connect_signal("property::floating", function(c) //FIXME: broken
--     if c.floating then
...
awesome.connect_signal("startup", function()
    for _,a in pairs(startup.commands) do
        pid = awful.spawn(a)
        table.insert(startup.pids, pid)
    end
end)
awesome.connect_signal("exit", function(reason_restart)
    if reason_restart then
        for _,pid in pairs(startup.pids) do
